...
public class GameServiceImpl implements GameService {
    ...
    public Game getGame(final String user) throws SQLException {
        ...
        if (!gameDAO.savedStateExists(user)) {
            ...
            List<Tile> rackHuman = rackService.getRack(bag, StringUtils.EMPTY, StringUtils.EMPTY);
            List<Tile> rackMachine = rackService.getRack(bag, StringUtils.EMPTY, StringUtils.EMPTY);
    ...
    @Override
    public void updateGame(final String user, final List<Character> bag, final List<Tile> humanRack) throws SQLException {
        Game prevGame = getGame(user);
        Game newGame = ImmutableGame.builder()
                .cells(prevGame.cells())
                .scoreHuman(prevGame.scoreHuman()).scoreMachine(prevGame.scoreMachine())
                .rackHuman(humanRack).rackMachine(prevGame.rackMachine())
                .bag(bag)
                .build();
        gameDAO.persistGame(user, newGame, UPDATE);
    ...
    public int processMachineMove(String user) throws SQLException {
        ...
        List<Tile> newRack = rackService.getRack(bag, rack, StringUtils.EMPTY);
